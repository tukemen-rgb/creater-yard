...
#
# 2026-08-19（O-4）: HTML から参照する URL には版（?v=<バイト数>）が付いた。
# **この検査は素の URL のまま引く。**ファイル名は変えていないので素の URL は
# 生きているし、素のほうが古いままなら版付きも古い可能性が高い＝先に気づける。
# なお「クエリを別物として扱うか」はうちの経路では未確認なので、
# 版付き URL に長い max-age を与えるのは**実測で確かめてから**（設計 O-4 の (b)）。
#
