...
### 配り方（2026-08-19・素材と権利は変えていない）

**HTML から参照する URL に版を付けた**（`?v=<バイト数>`・設計 O-4）。
ファイル名も中身も配布元も変えていないので、**上の表と権利確認は
そのまま効く**。変えたのは「同じ URL のまま古い複製が残らないようにする」
配り方だけ。版はビルド時に `fs.statSync` で読んだサイズなので、
**素材を差し替えれば勝手に変わる**（人が上げ忘れられない）。

なお `app/globals.css` の背景画像には版が付かない。CSS にはビルド時の値を
差し込めないため付けられない、というだけで、付け忘れではない
（理由は CSS のそばに書いてある）。
