...
/**
 * **保存しようとしたら、ログインが切れていたとき**（U-19）。
 *
 * 切れる道は 2 本ある —— **30 日で期限が来る**のと、
 * **他の端末でパスワードを変えた**（`tokenEpoch` が進む）のと。
 * **どちらかを人に見分けさせない。**できることは同じ「入り直す」だから。
 *
 * **書きかけを端末に預けてから登録へ送る形（U-15）は、ここでは採らない。**
 * U-18 で端末に残すものを減らすと決めた直後に増やす向きになり、しかも
 * 預ける先は**退会で消す対象にしたばかり**のものだからである。
 * 代わりに**画面を動かさない** —— 別のタブで入り直せば、
 * `lib/api.ts` は保存のたびにトークンを読み直しているので、
 * **もう一度押すだけで通る。**
 */
export const DEVICE_STORAGE_SESSION_EXPIRED =
  'ログインが切れました。この画面のまま、別のタブでログインし直してから、' +
  'もう一度 [保存] を押してください。書いた内容はこの画面に残っています。'
