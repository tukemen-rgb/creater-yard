 ...
 *
 * **2026-08-20 訂正。**この試験を足したとき「`api.mjs` を HTTP として
 * 起動している試験は 1 つも無い」と書いたが、**誤りだった。**
 * `server/test.mjs` に「HTTP: 登録 → 投稿 → 読む → 直す → 退会まで」が
 * 前からあり、`CY_API_AUTOSTART=0` で読み込んで `server.listen(0)` で
 * 起こしている。**下書きが他人には 404 であることも、そこで見ていた。**
 *
 * **測定の結果に出ていたのに、自分の但し書きを信じて読み落とした**
 * （grep は `server/test.mjs` を列挙していた）。
 *
 * では、この試験に何が残るか —— **3 通りを畳めているか**である。
 * 既存の通し試験は「他人には 404」までを見ており、
 * **その 404 が「無い記録」と同じ本文か**、**壊れたトークンでも同じか**は
 * 見ていない。ここで守っている約束は、この製品でいちばん外に出ては
 * いけないものである:
 *
