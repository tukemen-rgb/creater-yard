 ...
 */
/**
 * **この試験が守っていない範囲**（④ 15:10 の指摘 1・⑤ 15:30 の裁定 2）。
 *
 * これが止めるのは **この 1 つの文字列の再発だけ**である。
 * 「連絡先が無いのに連絡を促す」という**性質は縛っていない** ——
 * ④が実測したとおり、`お問合せ`（い を抜く）は**素通りする**。
 *
 * **わざと広げていない。**`問い合わせ`・`連絡先` を広く禁じると、
 * `CY_CONTACT_EMAIL` が決まって**確認画面から連絡先へ導線を張るとき**
 * （①の案 γ）に、**本当の直しのほうが赤くなる**。
 * **広い禁止は、いま守るものより、あとで来る直しを強く邪魔する。**
 */
